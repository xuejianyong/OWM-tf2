...
import tensorflow.compat.v1 as tf  # xue
tf.disable_eager_execution()       # xue
...
for j in range(task_num):
    ...
    for i in range(2000):
        batch_xs, batch_ys = mnist_list[j].train.next_batch(250)
    ...
    for i in range(j + 1):
        ...
        res_correct += res
        count_num += mnist_list[i].test.labels.shape[0]
        # print(res / mnist_list[i].test.labels.shape[0])
    print('Test on Previous All Datasets: {:.2f} %'.format(res_correct / count_num * 100))
